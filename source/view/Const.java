package source.view;

public interface Const {
    public static final String MAIN_BACKGROUNG = "images/background.jpg";
    public static final String BACK_ARROW = "images/back.png";
    public static final String LOAD_BACKGROUND = "images/load.jpg";
    public static final String GENERATE_BACKGROUND = "images/generate.jpg";
    public static final String ROLES_BACKGROUND = "images/roles.jpg";
    public static final String THE_MACABRE_FONT = "font/TheMacabre.otf";
    public static final String ROLES_FILE = "roles.txt";
    public static final int WINDOW_WIDTH = 1760;
    public static final int WINDOW_HEIGHT = 1080;
    public static final Integer BACKGROUND_CONSTRAINT = Integer.valueOf(0);
    public static final Integer FOREGROUND_CONSTRAINT = Integer.valueOf(1);
}
